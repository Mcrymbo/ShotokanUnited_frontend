...
export default function SingleEventPage() {
  ...
  const slug = newsItem?.registration_link?.split('/').pop();
  ...
  const handleNewsDetails = async () => {
    try {
      setLoading(true);
      const response = await auth.get(`/api/events/${id}`);
      ...
      <div className="bg-gray-50 min-h-screen">
        <div className="px-4 py-8">
          <div className="grid grid-cols-1 md:grid-cols-[1.5fr_2.5fr_1fr] gap-x-4 gap-y-8">
            ...
            <div className='relative'>
              <div className="top-4 md:h-full">
                ...
                <div className="flex items-center gap-2 p-2 md:absolute md:bottom-0">
                  ...
                  <span className="font-medium">{newsItem?.posted_by}</span>
                </div>
              </div>
            </div>
            ...
            <div>
              <div className="bg-inherit p-6 md:h-full">
                ...
                <p className='font-semibold text-sm text-end'>
                     {newsItem?.date ? (() => {
                        ...
                        if (minutes < 0){
                            return `${days * -1} days: ${hours * -1} hours: ${minutes * -1} minutes Remaining`;
                        } else {
                ...
                {newsItem?.registration_link ? (
                <Link
                    className="px-4 py-2 bg-orange-500 text-white font-semibold rounded-sm hover:bg-orange-400"
                    to={`/events/${id}/${slug}`}
                >
                    Register
                </Link>
                ) : (
                <span className="text-gray-500">Registration link unavailable</span>
                )}
              </div>
            </div>
